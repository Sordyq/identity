    ...
    async withdraw(user_id: string, dto: WithdrawDto) {
        ...
        if (!user.privateKey) {
            return {
                statusCode: "01",
                status: 'error',
                message: 'No private key stored for this user',
            }
        }

        // 2) Fetch wallet row (with accountId)
        const wallet = await this.prisma.wallet_tb.findUnique({
            where: { user_id },
        });
        if (!wallet) {
        ...
        // 3) Compare balances using Decimal
        ...
        // 5) Get Hedera accountId from wallet
        const userAccountId = wallet.accountId;
        if (!userAccountId) {
            return {
                statusCode: "01",
                status: 'error',
                message: 'User Hedera accountId not present',
            };
        }
        ...
        try {
            ...
            client.setOperator(
                ...
                PrivateKey.fromString(user.privateKey),
            );

            // 8) Create transfer tx
            ...
            // 11) On success: update balance + tx
            ...
            try {
                ...
                return {
                    statusCode: "01",
                    status: 'error',
                    message: 'Failed to update transaction status to Failed',
                }
            }
            return {
                statusCode: "01",
                status: 'error',
                message: `Withdrawal failed: ${err?.message ?? err}`,
            };
        }
    }
